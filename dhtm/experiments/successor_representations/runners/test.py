...
import dhtm.envs.gridworld
from dhtm.common.config.base import read_config, override_config
from dhtm.common.run.argparse import parse_arg_list
...
from dhtm.experiments.successor_representations.runners.agents import ECAgent
...
class ICMLRunner(BaseRunner):
    ...
    def make_agent(agent_type, conf):
        if agent_type == 'bio':
            from dhtm.experiments.successor_representations.runners.agents\
    ...
    def make_environment(env_type, conf, setup):
        if env_type == 'pinball':
            from dhtm.experiments.successor_representations.runners.envs import PinballWrapper
            env = PinballWrapper(conf, setup)
        elif env_type == 'animalai':
            from dhtm.experiments.successor_representations.runners.envs import AnimalAIWrapper
            env = AnimalAIWrapper(conf, setup)
        elif env_type == 'gridworld':
            from dhtm.experiments.successor_representations.runners.envs import GridWorldWrapper
    ...
    def reset_model(self, checkpoint_path=None):
        if isinstance(self.agent.agent, ECAgent):
            self.agent.agent.reset_model()
        else:
            layer = self.agent.agent.cortical_column.layer
            if isinstance(layer, LstmLayer) or isinstance(layer, RwkvLayer):
    ...
    def state_visited(self):
        ...
        assert isinstance(env, dhtm.envs.gridworld.GridWorld)
        r, c = env.r, env.c
